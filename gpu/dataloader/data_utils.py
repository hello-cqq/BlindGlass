...
def point_dist_to_line(p1, p2, p3):
    ...
    dist = np.linalg.norm(np.cross(p2 - p1, p1 - p3))*1.0 / (np.linalg.norm(p2 - p1)+1e-5)
    return dist
...
def generate_rbox(im_size, polys, tags):
    ...
    geo_map = np.zeros((4,h, w), dtype=np.float32)
    agl_map = np.zeros((h, w), dtype=np.float32)
    ...
    for poly_idx, poly_tag in enumerate(zip(polys, tags)):
        ...
        for y, x in xy_in_poly:
            ...
            # top
            geo_map[0, y, x] = point_dist_to_line(p0_rect, p1_rect, point)
            # right
            geo_map[1, y, x] = point_dist_to_line(p1_rect, p2_rect, point)
            # down
            geo_map[2, y, x] = point_dist_to_line(p2_rect, p3_rect, point)
            # left
            geo_map[3, y, x] = point_dist_to_line(p3_rect, p0_rect, point)
            # angle
            agl_map[y, x] = rotate_angle
    return score_map, geo_map,agl_map, training_mask
